    ...
    fun loadBanksRates(bank: Bank, fromCurrency: Currency, toCurrency: Currency): Completable {
        return if (places.isEmpty()) {
            ...
            Single.just(emptyList())
                ...
                .flatMap { places ->
                    repository.getRatesOfBranch(fromCurrency, toCurrency)
                            .doOnSuccess { ratesOfBranch ->
                                this.places = places

                                database.runInTransaction {
                                    database.saveBranches(places)
                                    database.updateBranches(ratesOfBranch)
                                    database.saveExchangeRates(ratesOfBranch.flatMap { it.exchangeRates })
